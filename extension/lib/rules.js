// rules.mjs 에서 이식 (무수정). 혈액/UA 요약 포맷 로직.
...
function formatCombinedSingle(label, firstName, secondName, recent) {
  return `${label}(${valueWithFlag(recent.lab.get(firstName))}/${valueWithFlag(recent.lab.get(secondName))})`;
}

export function buildBloodSummaryRecent(recentRows) {
  const recent = indexRows(recentRows);
  const empty = { lab: new Map(), ua: new Map() };
  const labItems = [
    formatSingle("WBC", recent.lab.get("WBC")),
    formatSingle("CRP", recent.lab.get("CRP (정량)")),
    formatCombinedSingle("Na/K", "Sodium (Na)", "Potassium (K)", recent)
  ];

  if (shouldShowCombined("BUN", "Creatinine", recent, empty)) {
    labItems.push(formatCombinedSingle("BUN/Cr", "BUN", "Creatinine", recent));
  }
  if (shouldShowCombined("AST (GOT)", "ALT (GPT)", recent, empty)) {
    labItems.push(formatCombinedSingle("OT/PT", "AST (GOT)", "ALT (GPT)", recent));
  }

  const names = new Set([...recent.lab.keys()]);
  for (const name of [
    "WBC", "CRP (정량)", "Sodium (Na)", "Potassium (K)",
    "BUN", "Creatinine", "AST (GOT)", "ALT (GPT)"
  ]) names.delete(name);
  ...
  for (const name of sortByOrder([...names], LAB_ORDER)) {
    const recentRow = recent.lab.get(name);
    if (!shouldShowLab(name, recentRow, undefined)) continue;
    labItems.push(formatSingle(displayLabel(name, recentRow), recentRow));
  }

  return wrapItems("Lab 결과", labItems);
}

export function buildUaSummaryRecent(recentRows) {
  const recent = indexRows(recentRows);
  const uaItems = [];
  const uaNames = sortByOrder([...recent.ua.keys()], UA_ORDER);
  for (const name of uaNames) {
    const recentRow = recent.ua.get(name);
    if (!isUaAbnormal(recentRow)) continue;
    uaItems.push(formatSingle(UA_ABBR.get(name) || name, recentRow));
  }
  if (!uaItems.length && uaNames.length > 0) return "UA 결과 : Normal";
  return wrapItems("UA 결과", uaItems);
}
